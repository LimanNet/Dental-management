...
function login() {
	...
	if (!$_REQUEST){
		...
		if($doc){
			...
			$_SESSION['idDoctor'] = $doc['id'];
			$expire=time()+60*60*24*30;
			
			setcookie("user", $doc['familyName'], $expire);
			$log = ' Вход в систему: '.$doc['id'].$doc['familyName'];
			logMes($log);
...
function calendar(){
	...
	require_once( TEMPLATE_PATH . "/Calendar/index.php" );
}
...
function patients() {
	...
	require_once( TEMPLATE_PATH . "/Patient/index.php" );
}
  ...
  */
function catalog(){
	require_once( '/function/catalog.php' );
}

function logMes($mes){
	$mes = '\n'.date("m.d.y").': '.$mes.'\n';
	file_put_contents('../Log.txt', $mes, FILE_APPEND | LOCK_EX);
}
