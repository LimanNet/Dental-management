...
class VitaColor{
	...
	function getAll(){
		...
		$STH = $DBH->prepare('SELECT * FROM clinicbase.vita_color ORDER BY id;');
		$STH->execute();
		$result = $STH->fetchAll();
		$DBH = null;
	...
	function insert($array = array()){
		...
		$total = count( $insert );
		$sql = 'INSERT INTO  clinicbase.vita_color (';
				$counter = 0;
	...
	function delete(){
		...
		$DBH->exec("delete from clinicbase.vita_color where id='".$this->id."';");
