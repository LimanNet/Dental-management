...
class ToothPart{
	...
	function getAll(){
		...
		$STH = $DBH->query('SELECT * FROM clinicbase.tooth_part WHERE archived IS NULL ORDER BY name;');
	...
	function archive(){
		...
		$STH = $DBH->query('SELECT * FROM clinicbase.tooth_part WHERE archived IS NOT NULL ORDER BY name;');
		$result = $STH->fetchAll(PDO::FETCH_ASSOC);
		$DBH = null;
	...
	function insert(){
		...
		$total = count( $insert );
		$sql = 'INSERT INTO  clinicbase.tooth_part (';
				$counter = 0;
	...
	function delete(){
		...
		$DBH->exec("DELETE FROM clinicbase.tooth_part WHERE id='".$this->id."';");
