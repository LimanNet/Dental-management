...
class Print043{
	...
	function getAll(){
		...
		$STH = $DBH->query('SELECT * FROM clinicbase.print043 ORDER BY name;');
	...
	function insert($array = array()){
		...
		$total = count( $insert );
		$sql = 'INSERT INTO  clinicbase.print043 (';
				$counter = 0;
	...
	function delete(){
		...
		$DBH->exec("delete from clinicbase.print043 where id='".$this->id."';");
