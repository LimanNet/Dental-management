...
class Diagnosis_type{
	...
	function getAll(){
		...
		$STH = $DBH->query('SELECT * FROM clinicbase.diagnosis_type ORDER BY name;');
	...
	function insert($array = array()){
		...
		$total = count( $insert );
		$sql = 'INSERT INTO  clinicbase.diagnosis_type (';
				$counter = 0;
	...
	function update(){
		...
		$query = '
			UPDATE clinicbase.diagnosis_type
				SET ';
				$total = count($a);
				$counter = 0;
				foreach ($a as $param=>$value) {
	...
	function delete(){
		...
		$DBH->exec("delete from clinicbase.diagnosis_type where id='".$this->id."';");
