...
class IllDestination{
	...
	function getAll(){
		...
		$STH = $DBH->query('SELECT * FROM clinicbase.ill_destination WHERE archived IS NULL ORDER BY name;');
	...
	function archive(){
		...
		$STH = $DBH->query('SELECT * FROM clinicbase.ill_destination WHERE archived IS NOT NULL ORDER BY name;');
		$result = $STH->fetchAll(PDO::FETCH_ASSOC);
		$DBH = null;
	...
	function insert(){
		...
		$total = count( $insert );
		$sql = 'INSERT INTO  clinicbase.ill_destination (';
				$counter = 0;
	...
	function update(){
		...
		$query = '
			UPDATE clinicbase.ill_destination
				SET ';
				$total = count($a);
				$counter = 0;
				foreach ($a as $param=>$value) {
	...
	function delete(){
		...
		$DBH->exec("DELETE FROM clinicbase.ill_destination WHERE id='".$this->id."';");
